...
class ImageRepositoryImpl(
    private val imageCollection: StorageReference = FirebaseStorage.getInstance().reference.child(FirebaseReferenceEnum.PROFILE_IMAGES),
    ...
    override suspend fun uploadImg(uid: String, imgUri: Uri): String {
        return withContext(ioDispatchers) {
            ...
            imageCollection.child(uid).putFile(imgUri).addOnSuccessListener {
                Log.d(TAG, "Image is uploading")
            }.addOnFailureListener {
                Log.d(TAG, "Image could not be uploaded")
            }.await()
            val link = imageCollection.child(uid).downloadUrl.addOnSuccessListener {
            }.addOnFailureListener {
                Log.d(TAG, "Image could not be uploaded")
            }.await().toString()
            link
